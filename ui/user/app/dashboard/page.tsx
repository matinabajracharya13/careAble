...
function CareerDashboard({ user }: { user: NonNullable<ReturnType<typeof useAuth>['user']> }) {
  ...
  const hasAssessmentData = heatmapData && heatmapData.length > 0;
  ...
  return (
    <div className='min-h-screen bg-background pt-20 pb-12'>
      <div className='container mx-auto px-4 max-w-6xl'>
        {user.role === 'employer' ? <EmployerDashboard user={user} /> : <CareerDashboard user={user} />}
      </div>
    </div>
  );
}